...
[KernelFunction("Get_Captial")]
   [Description("""
         Returns the Capital of India        
""")]
  public async Task<string> GetCaptital()
  {
    await Task.Delay(0);
    return "New Delhi is the Capital of India";
  }

public async Task IngestDataToChatStore(
        [Description("The collection name for cache.")] string cacheCollectionName,
        [Description("The ChatStore object to commit to the cache.")] ChatStore<TKey> newCacheEntry)
    {
        
        var collection = vectorStore.GetCollection<TKey, ChatStore<TKey>>(cacheCollectionName);
  ...
  //[KernelFunction, Description("Searches the vector store for Frequently Asked Questions FAQ")]
    public async Task<string> SearchChatStore(
        ...
        [Description("The number of top results to return.")] int topResults = 1)
    {
        try
        {
            var collection = vectorStore.GetCollection<TKey, ChatStore<TKey>>(collectionName);
            await collection.CreateCollectionIfNotExistsAsync();
...
//            var answers = results.Select(r => r.Answer).ToList();
            var answersWithScores = resultRecords.Select(r =>
            {
                ...
                return $"Recently Chat response from ChatStoreCache\n\n{r.Record.Answer} (Score: {score}, Matching Percentage: {percentage}%)";
            }).ToList();
...
//            var answers = results.Select(r => r.Answer).ToList();
     ...
     public async Task FAQIngestDataToVectorStore(IVectorStore vectorStore, string collectionName, Func<TKey> uniqueKeyGenerator)
    {
        try
        {
            var collection = vectorStore.GetCollection<TKey, FAQ<TKey>>(collectionName);
...
public sealed class ChatStore<TKey>
    {
        [VectorStoreRecordKey]
        public TKey QuestionId { get; set; }
